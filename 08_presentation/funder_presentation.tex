...
\begin{enumerate}\setlength\itemsep{10pt}
    \item \textbf{The dataset.} We built the first systematic corpus of District Attorney policy documents---1,865 memos from 41+ offices, coded by topic and ideological direction using an AI-assisted pipeline.
    \item \textbf{What it enables.} This measurement infrastructure supports 6+ credible causal designs linking prosecutorial policy changes to criminal justice outcomes, including diversion rates, pretrial detention, and sentencing.
    \item \textbf{What we need.} Funding to link outcome data, scale nationally, and convert pilot results into published causal evidence.
\end{enumerate}
\end{frame}
...
\end{columns}
\vspace{8pt}
{\small\emph{Our approach scales the logic of single-memo causal designs to systematic measurement across \textbf{41+ offices and 1,865 documents}, creating the variation needed for average causal effects and heterogeneity analysis.}}
\end{frame}
...
% ---------- BACKUP: FAQ (1/2) ----------
\begin{frame}{Frequently Asked Questions (1/2)}
\begin{block}{What is the identification strategy?}
...
\end{block}
\vspace{6pt}
\begin{block}{Do policy memos translate to practice?}
{\small Extension 2 directly tests this: do ``diversion-positive'' memos predict more diversions? Even the stated-policy variation is \emph{itself} a novel descriptive contribution.}
\end{block}
\end{frame}

% ---------- BACKUP: FAQ (2/2) ----------
\begin{frame}{Frequently Asked Questions (2/2)}
\begin{block}{How does the project generalize beyond California?}
{\small Extension 1 addresses this directly. CA has internal variation (Bay Area vs.\ Central Valley), but external validity requires the 100 largest DA offices nationally.}
\end{block}
\vspace{6pt}
\begin{block}{How reliable is the AI-based coding?}
