...
\begin{frame}[shrink=30]{Overcoming Pilot Limitations}
{\smalltable
\begin{tabular}{p{2.5cm}p{3.5cm}p{5.5cm}}
\toprule
\textbf{Limitation} & \textbf{Root Cause} & \textbf{Solution} \\
\midrule
COVID confounds DiD & All CA counties declined in 2020 & Multi-state data; synthetic control on pre-treatment \\[3pt]
Parallel trends violated ($p = 0.021$) & LA converging pre-Gasc\'on & Longer pre-period + case-level data \\[3pt]
TWFE non-significant ($N = 137$) & 34 counties, 9 years & National expansion increases $N$ \\[3pt]
Aggregate data too coarse & Cannot distinguish mechanisms & Case-level data: mechanism decomposition \\
\bottomrule
\end{tabular}}
\end{frame}

% ---------- BACKUP: Causal Strategies ----------
\begin{frame}[shrink=30]{Causal Identification Strategies}
{\smalltable
\begin{tabular}{p{2cm}p{3cm}p{2.8cm}p{2.8cm}}
\toprule
\textbf{Design} & \textbf{Current Status} & \textbf{What's Needed} & \textbf{Extension} \\
\midrule
RDD & $\sim$5 close CA elections & 30--50 close elections & Ext 1 (national) \\[3pt]
Stacked DiD & 9 disruptions; Vera only & More outcomes + transitions & Ext 1 + 2 \\[3pt]
Synthetic Control & Gasc\'on pilot works & More outcome data & Ext 2 \\[3pt]
Event Study & Pre-period marginal (6 yr) & Longer pre-period & Ext 1 \\[3pt]
...
\begin{frame}[shrink=30]{Extension 2 Detail: Outcome Data Sources}
{\smalltable
\begin{tabular}{p{2.3cm}p{4cm}lrl}
\toprule
\textbf{Source} & \textbf{Records} & \textbf{Timeline} & \textbf{Cost} & \textbf{Priority} \\
\midrule
CA DOJ OpenJustice & County-yr arrest, filing, incarceration & Immediate & Free & High \\[3pt]
CA Sentencing Comm. & Enhancement filings, 3-strikes & 1--2 mo & Free & High \\[3pt]
Vera (done) & Quarterly jail pop and admissions & Done & Done & Done \\[3pt]
UniCourt / PACER & Case dispositions, plea bargains & 2--3 mo & \$30--50k & Very high \\[3pt]
CJARS & Arrest $\to$ parole, 24 states & 3--12 mo & \$5k & Highest \\
\bottomrule
\end{tabular}}
\vspace{6pt}
{\small\textbf{Case-level data is the gold standard:} it links policy documents directly to line-prosecutor behavior. Are ``minimize enhancements'' memos actually reducing enhancement filings?}
\end{frame}

% ---------- BACKUP: Disruption Detection ----------
\begin{frame}[shrink=30]{Policy Disruption Detection: Methodology}
{\tablefont
\begin{tabular}{lrp{7cm}}
\toprule
\textbf{Signal} & \textbf{Weight} & \textbf{Method} \\
\midrule
Ideology Velocity & 30\% & Rate of ideology change vs.\ prior 2-year baseline \\[3pt]
Novelty Index & 25\% & Proportion of first-time policy types \\[3pt]
Topic Shift & 20\% & Jensen-Shannon divergence of topic distributions \\[3pt]
Margin Reversal & 15\% & Flips in extensive/intensive leniency direction \\[3pt]
...
\begin{frame}[shrink=30]{Anticipated Objections}
...
{\small Face validity is strong ($d = 0.75$ Gasc\'on, geographic clustering). Phase 1 includes 200-doc human validation for gold-standard IRR. Cost: \$80, runtime: 2 hours.}
\end{block}
\end{frame}
